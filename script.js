...
transactionForm.addEventListener('submit', (event) => {
  ...
  const date = document.getElementById('date').value; // Get date value if provided
  ...
  const transaction = {
    ...
    category,
    description,
    date: date || null, // Set date if provided, otherwise null
  };
...
function displayTransactions(transactions) {
  ...
  for (const category in transactionsByCategory) {
    ...
    transactionsByCategory[category].forEach((transaction) => {
      ...
      listItem.innerHTML = `${'&#10022;'} ${icon} ${transaction.type}: ${transaction.amount} Da - ${transaction.description} - ${transaction.date} `;
